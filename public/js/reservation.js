  ...
  allReservationCard.forEach((reservationCard) => {
    ...
    updateBtns.forEach((button) => {
      button.addEventListener("click", () =>
        updateQuantityReservation(reservationCard, button.name)
      );
    });
  });
});
...
const updateQuantityReservation = async (card, actionType) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Erreur avec la reservation ${reservationId}: ${response.status}`
      );
    }
    ...
    if (data.total_reservation.number_of_reservation === 0) {
      const titleReservation = document.querySelector(
        ".pr-reservation-container h1"
      );
